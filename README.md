...
| 🍎 **Mac** (macOS 11 или новее) | **[Скачать HHsim для Mac](https://github.com/rem-aster/HHsim/releases/latest/download/HHsim-3.7-ru-macOS.dmg)** (≈ 1 МБ) |
...
1. Откройте скачанный файл **HHsim-3.7-ru-macOS.dmg**. Появится окно со значком HHsim и
   папкой «Программы».
2. Перетащите значок **HHsim** на папку **«Программы»**. Окно можно закрыть.
3. Откройте **HHsim** из папки «Программы» (или через Launchpad).
   Mac предупредит, что не может проверить разработчика, — нажмите **«Готово»**.
   Затем откройте **меню Apple** (значок яблока в левом верхнем углу экрана) →
   **«Системные настройки»** → **«Конфиденциальность и безопасность»**, прокрутите вниз и
   нажмите **«Всё равно открыть»** рядом с HHsim; Mac попросит подтвердить паролем или
   Touch ID. Это нужно сделать только один раз: программа бесплатная и не покупала
   «подпись» у Apple.
   *(На macOS 14 и старше проще: щёлкните по HHsim правой кнопкой мыши → «Открыть» → «Открыть».)*
4. При первом запуске HHsim предложит скачать бесплатную среду **GNU Octave**, в которой он
   работает. Нажмите **«Скачать»** и подождите 5–15 минут (нужен интернет, около 500 МБ).
   Ничего вводить не нужно — когда всё будет готово, HHsim откроется сам.

Дальше HHsim запускается сразу, как обычная программа.
...
* **Mac: «HHsim повреждён и не может быть открыт».** Удалите HHsim из «Программ»,
  снова откройте скачанный файл .dmg и перетащите HHsim в «Программы» ещё раз, затем
  выполните шаг 3 инструкции.
* **Mac: не удалось скачать Octave.** Проверьте интернет и откройте HHsim ещё раз —
